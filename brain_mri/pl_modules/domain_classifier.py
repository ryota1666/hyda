...
class DomainClassifier(pl.LightningModule):
    ...
    def forward(self, x, return_feats=False):
        ...
        if return_feats:
            feats = self.classifier.model[:-1](feats.flatten(1))
            return feats
        else:
            domain_logits = self.classifier.model(feats.flatten(1))
            return domain_logits

    def _step(self, batch, return_feats=False):
        scan, domain, age, age_bin, subject = batch
        domain_labels = self.gt2train_ids[domain]
        domain_out = self(scan, return_feats)
